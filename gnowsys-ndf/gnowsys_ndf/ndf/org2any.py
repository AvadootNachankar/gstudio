...
def org2pdf(org_content, file_prefix="", file_delete=True):
  try:
    # org editor content manipulation for temporary file (".org")
    org_content_header_for_file = "\n#+OPTIONS: timestamp:nil author:nil creator:nil ^:{} H:3 num:nil toc:nil @:t ::t |:t ^:t -:t f:t *:t <:t" \
                                  + "\n#+TITLE: \n"
    ...
    org_file_obj.write(encode_content)
    # NOTE: Don't close this file till the time, pdf file is created

    # Move the cursor - pointing to start of the file
    # Must, otherwise cursor remains @ end (as this file isn't closed after writing data into it) and you won't get any data
    org_file_obj.seek(0)

    # Exporting the above created ".org" file to pdf
    # Example (filename_pdf): "/tmp/wiliname-usrname-tmptCd4aq.pdf"
    ext_pdf = ".pdf"
    filename_pdf = filename_org[:-4] + ext_pdf
    ...
    if (float(org_version) >= 8): 
      export_to_pdf="org-pdf-export-to-pdf"
    else:
      export_to_pdf="org-export-as-pdf"

    # Batch command that converts org-file into corresponding pdf-file
    cmd = "emacs -l " + EMACS_INIT_FILE_PATH  + " --batch " + filename_org + " --eval '(" + export_to_pdf + " nil)'"
    cmd_res = call((cmd + ' </dev/null'), shell=True)
